...
- `web_factor/upstream-sync.json` 已更新到 v1.5.0 / web tree，本次发版纳入 AntD `6.5.4` 依赖更新。

验证结果：

- `pnpm --dir web_factor test`：40/40 通过；`pnpm --dir web_factor build`：通过。
- `pnpm --dir web build`、`make web-all`：通过。
- `antd lint web_factor/src --version 6.5.4 --format json`：0 个问题；Factor 测试、构建和 lint 均按 AntD `6.5.4` 工作树版本通过。
